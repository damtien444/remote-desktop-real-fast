...
class Serving {
    ...
    // giữ các bản ghi địa chỉ của các client

    public Serving(){
        try {
            runServer();
        } catch (IOException ex) {
            Logger.getLogger(Serving.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    public Serving(int userTcpPort, int userUdpPort){
    ...
    public static void main(String[] args) throws IOException {
        if (args.length>0) {
            new Serving(Integer.parseInt(args[0].trim()), Integer.parseInt(args[1].trim()));
        }else{
            new Serving();
        }
